...
value = str(2_000_000_000_000_000_000) # 0.00001 BNB (1 BNB = 0.000000000000000001 wei)
...
async def evmTxNative():
    try:
        ...
        #print("resp_tx.text: ", resp_tx.text)
        #print("resp_tx.text.id: ", resp_tx.text.id)
    except Exception as e:
        print(f"❌ Transaction failed: {str(e)}")

async def rawTx(hexData, _to, _chain):
    print("calling evmTxRaw.rawTx ")
    print("hexData, _to, _chain: ", hexData, _to, _chain)
    print()
    try:
        request_json = await evm_tx_raw(
            vault_id=EVM_VAULT_ID,
            chain= _chain,	#"evm_ethereum_mainnet",  # or polygon_mainnet
        ...
        #print("resp_tx.json: ", resp_tx.json)
        #print("resp_tx.text: ", resp_tx.text)
        #print()
    except Exception as e:
        print(f"❌ Transaction failed: {str(e)}")
    ...
    if (txIdData['state']!="completed"): return "fail"
    return txHash
