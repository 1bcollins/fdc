...
def run(droidId, stableCoinPos):
	...
	print(f"Avg Removed Price: {removesResults["weighted_average"]:.4f}")
	print(f"Avg Add Price: {additionsResults["weighted_average"]:.4f}")
	print(f"Additions Purchase Delta: {watchCoinPurchasePriceDelta:.4f}")
	print(f"Watch Coin Removed: {removesResults['amount']}")
	print(f"Watch Coin Added: {additionsResults['amount']}")
	...
	print(f"Stable coin Accumulated: {stableCoinDiff}")
