...
from utils.ladder_utils import get_ladder_structure
from utils.trigger_utils import getTriggerStat
...
def upsert_droid_status(droid, nftArray, nftsLiquUSD, lpPosArray, botArray, ratioArray, cursor, connection):
    ...
    # Compute ladder structure
    ladderArray = get_ladder_structure(droid, cursor, LpPositionsStat)
    ladderArray_json = json.dumps(ladderArray)

    # Safe access to DROID_SESSION
    session = DROID_SESSION.get(droidId, {})
    risingRebalTrigg = session.get('risingRebalanceTrigger', 0.00) or 0.00
    fallingRebalTrigg = session.get('fallingRebalanceTrigger', 0.00) or 0.00
...
def check_breakers(droid, session) -> str | None:
	...
	watch_price = float(LpPoolStat['token0']['tokenDayData'][0]['priceUSD'])	#TODO: change to dynamic
...
def check_duplicate_lp_positions(droid_id, cursor):
	...
	query = """
		...
		FROM PositionBots
		WHERE droidId = %s AND lpPositionId IS NOT NULL AND lpPositionId<>0 
...
def addPositionToDB(nftNumber, nftArray, nftsLiquUSD, cursor, botArray, connection, droid):
	...
	if unFundedNftIndex != -1:
		...
		print("found lpPosRecord: ", lpPosRecord)
		print()
		if (lpPosRecord==None):
			print("LpPositionsStat['lpPool']['id']", LpPositionsStat['lpPool']['id'])
			print()
...
def iniDroid(droid, cursor, connection):
	...
	#print()
	now=int(time.time())
	session_logger.info(f"Session Status @ {now}: Calling 'iniDroid'")
	session_logger.info(f"Session Status @ {now}: Droid data={droid}")
...
def getTriggerStat(droid, cursor) -> str:
	...
	#add update to DROID_SESSION data here for risingRebalanceTrigger and fallingRebalanceTrigger
...
def moveCenter(droid, existingNft, cursor):
	...
	if existingNftFundingUSD > 0:
		...
		if (movedToBot==0):
			print("!! NOTE: movedToBot == 0 !!")
			print("Make trap here!")
...
def centerRebalance(droid, cursor, connection):
	...
	if existingNft == 0:
		...
		print("Getting Prev LP Postion ID!!")
...
def ladderDn(droid, cursor, connection):
	...
	try:
		ladder_structure = get_ladder_structure(droid, cursor, LpPositionsStat)
	except Exception as e:
...
def getMaxGasGwei(funding_usd, expected_fee_per_1000=15, gas_budget_ratio=0.25, estimated_gas_units=290_000):
	...
	maxGasPriceOveride=50	#TODO: FIX THIS to operator control
...
def run(droidId):
	...
	while True:
		try:
			...
			now=int(time.time())
			session_logger.info(f"Restarting @ {now}: with error={e}")
			DROID_SESSION.clear()	#={}
...
def main(DROID_ID):
	...
	while True:	#Note: Loop Is Pool specific and Wallet specific.
		droid = getDroid(DROID_ID, cursor)
		if(droid['active']==2):
			print("Manual reset of droid found!")
			print()
			#set droid['active'] back to 1
			cursor.execute("UPDATE Droids SET active = 1 WHERE id = %s", (droid['id'],))
			connection.commit()
			session_logger.info("Manual Remote Reset Found")
			raise ExceptionType("Manual Remote Reset Found")
		...
		if(droid['centerPosBotId']!=0): 
			...
			triggerStat = getTriggerStat(droid, cursor, LpPositionsStat, LpPoolStat, DROID_SESSION, previous_values, OWNER)
...
if __name__ == "__main__":
	droidId=0
	if len(sys.argv)>1: droidId = int(sys.argv[1])
