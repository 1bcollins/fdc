...
def addPositionToDB(nftNumber, nftArray, nftsLiquUSD, cursor, botArray, connection):
...
def alignDBwithBC(nftArray, nftsLiquUSD, cursor, botArray, connection):
	...
	for pos in LpPositionsStat['positions']:
		if pos['active']:
			...
			if not nftFound:
				...
				addedPosition = addPositionToDB(pos['nftNumber'], nftArray, nftsLiquUSD, cursor, botArray, connection)
...
def iniDroid(droid, cursor, connection):
	...
	if(droid['centerPosBotId']==0): 
		...
		addedPosition=alignDBwithBC([0], [0], cursor, [newBotId], connection)	#align the database positions found with blockchain function
...
def swapRoutine(amountNeeded, tokenNeeded):
...
def buildMintParams(poolId, tick_lower, tick_upper, liquUSD, cursor):
	import time
...
def centerRebalance(droid, cursor, connection):
	...
	if existingNft == 0:
		...
		#            buildMintParams(poolId,           tick_lower,   tick_upper,  liquUSD, cursor, lp_pool_stat, lp_positions_stat)
		mintParams = buildMintParams(droid['poolId'], newRangeLow, newRangeHigh, centerNftFundingUSD, cursor)	#, LpPoolStat, LpPositionsStat)
...
def handle_new_mint_position(droid, ladder_index, tickLower, tickUpper, botId, cursor, connection):
	...
	#buildMintParams(poolId, tick_lower, tick_upper, liquUSD, cursor, lp_pool_stat, lp_positions_stat)
	mintParams = buildMintParams(droid['poolId'], tickLower, tickUpper, fundingUSD, cursor)	#, LpPoolStat, LpPositionsStat)
...
def pruneDroid(droid, cursor, connection):
	...
	if(inRange<0 and gasCheck):
		...
		try:
			...
			print()
			if test=="y":
				receipt=lpPositionLiquidate.remLiquAndCollFordefi(inftArray[max_dev_index], CHAIN)
				if(receipt=="fail"): return 0
				updateLpTxs(droid['id'])
		except Exception as e:
...
def update_droid_controller_session(cursor, connection):
	for droid_id, session_data in DROID_SESSION.items():
		...
		if not session_id:
			print(f"❌ No sessionId for droidId {droid_id}, skipping update.")
			continue
		...
		try:
			...
			connection.commit()
			print(f"✅ Updated DroidControllerSession ID {session_id} for Droid {droid_id}")
		except Exception as e:
			print(f"❌ Failed to update session ID {session_id} for Droid {droid_id}: {e}")
'''	
