...
def editRebalanceTrigg(droid_id):
	print()
	print("Edit rebalance trigger here!")
	print("Options: ")
	print("	1. Auto")
	print(" 2. Manual")
	choice = input("Enter option: ").strip()
	if choice=="1":
		print("AUTO feature selected for Droid Rebalance calcuation points!")
		#update DB fields to -1 for fallingRebalanceTrigger and risingRebalanceTrigger
		update_droid_field(droid_id, "fallingRebalanceTrigger", -1.00)
		update_droid_field(droid_id, "risingRebalanceTrigger", -1.00)
		print("Droid Rebalance Calculator is set to 'AUTO'!")
		return
	if choice=="2":
		print("MANUAL feature selected for Droid Rebalance calcuation points!")
		#get settings per user
		fallingPerc=float(input("Enter 'Low' percent of range to rebal at (example: .25 for 25%)"))
		risingPerc=float(input("Enter 'High' percent of range to rebal at (example: .75 for 75%)"))
		#check settings to be in bounds
		if fallingPerc>1 or fallingPerc<0 or risingPerc>1 or risingPerc<0 or fallingPerc>risingPerc:
			print("Error: input(s) are out of bounds!")
			return
		else:
			print("Updating 'Rebalance fields!")
			#update DB fields to 'user_settings' for fallingRebalanceTrigger and risingRebalanceTrigger
			update_droid_field(droid_id, "fallingRebalanceTrigger", fallingPerc)
			update_droid_field(droid_id, "risingRebalanceTrigger", risingPerc)
			return
...
def modify_droid_field(droid_id):
	...
	if (field_name=="rebalTriggType"):
		editRebalanceTrigg(droid_id)
		return
